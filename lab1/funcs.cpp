...
void lab1::print_matrix(Matrix &matr) {
    //if (!matr) return;
        List *ptr = matr.rows;
        Node *tmp;
        while (ptr) {
...
int lab1::add_el(Matrix &matr, int row, int column, double val) {
    if (row >= matr.n || column >= matr.m) {
        return 1;
    }
   Node *a;
    List *l;
...
void lab1::find_el(List &l, double &max, double &min, int &count) {
    if (!l.head) return;
    Node *ptr = l.head;
    int count_max=0;
    int count_min=0;
